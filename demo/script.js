...
(async () => {
    const cover = await fetch("./cover-image.jpg", { mode: "no-cors" }).then(
        (response) => {
    ...
    jepub.image(cover, "1234");
    jepub.add("Imagem", "<p> <%= image[1234] %> </p>");

    let illustration = await fetch("./lorem-ipsum.jpg", {
        mode: "no-cors",
    }).then((response) => {
